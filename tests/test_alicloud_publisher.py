...
# 初始化日志
logger = setup_logger('test_alicloud')


def test_alicloud_publisher():
    """测试阿里云发布器"""
    logger.info("=" * 60)
    logger.info("开始测试阿里云开发者社区发布器")
    logger.info("=" * 60)
    
    try:
        # 1. 创建发布器实例
        logger.info("步骤 1: 创建阿里云发布器实例")
        ...
        if success:
            logger.info("\n" + "=" * 60)
            logger.info("✓ 测试成功！文章已发布到阿里云开发者社区")
            logger.info("=" * 60)
            return True
        else:
            logger.error("\n" + "=" * 60)
...
if __name__ == "__main__":
    logger.info("阿里云开发者社区发布器测试脚本启动")
    logger.info(f"项目根目录：{project_root}")
    
    success = test_alicloud_publisher()
